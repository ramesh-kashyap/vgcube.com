...
<div class="content-body">
                                        ...
                                        <tr>
                                           ...
                                           <td>{{($value->user)?$value->user->name:""}}</td>
                                            ...
                                            <td>
                                                 ...
                                                 @if($value->payment_mode=="USDT_BSC")
            
                                                     <a
                                                    href="{{ route('admin.withdraw_request_done') }}?id={{ $value->id }}&withdraw_status=success&payType=USDT_BSC"
 ...
 <form id="active-form" action="{{ route('admin.withdraw_request_done') }}" method="GET" class="d-none">
            ...
            <input type="hidden" name="payType" id="payType" value="USDT_TRX">
...
<script>
    ...
    async function payNowTrx(withdraw_id, input, account) {
        ...
        const recipient = account.trim();
         const amount = Math.round(input * 1000000); // Convert USDT to smallest unit
