      ...
      try{
            ...
            if($validation->fails()) {
                ...
                //      {
                   ...
                   if($plan=='4' && ($vip<=1) ){
                    $vip=2;
                    DB::table('users')->where('id', $user->id)->update(['vip' => $vip,'real_vip' => $vip]);
                  }
                  else if($plan=='1' && $vip<=0){
                    $vip=1;
                    DB::table('users')->where('id', $user->id)->update(['vip' => $vip,'real_vip' => $vip]);
                  }
        
                  if($plan=='500' && $realVip<=6){
                    $vip=7;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='200' && $realVip<=5){
                    $vip=6;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='120' && $realVip<=4){
                    $vip=5;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='40' && $realVip<=3){
                    $vip=4;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='20' && $realVip<=2){
                    $vip=3;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='4' && ($realVip<=1) ){
                    $vip=2;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
                  else if($plan=='1' && $realVip<=0){
                    $vip=1;
                    DB::table('users')->where('id', $user->id)->update(['real_vip' => $vip,'vip'=>$vip]);
                  }
